...
/*   Updated: 2021/01/26 15:39:38 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*replace_err_code(char *arg, int begin)
{
	...
	i = 0;
	j = 0;
	c = ft_itoa(g_status.code);
	result = malloc((ft_strlen(arg) + (ft_strlen(c) - 1) + 1) *sizeof(char));
	if (!result)
		return (0);
	replace_code(result, arg, c, begin);
	free(c);
	return (result);
}
...
char	*replace_var(t_minish *mini, char *arg, int begin, int env_index)
{
	...
	len_diff = ft_strlen(mini->parsed_env[env_index].value)
		- var_end_dist(arg, begin);
	...
	while (arg[i])
	{
		if (arg[i] == -36 && quotes(arg, i) != 2)
		{
			if (arg[i + 1] == '?')
				...
				result = replace_var(mini, arg, i,
						exp_in_env(mini, arg, i + 1));
			else
				result = delete(arg, i);
			break ;
		}
